...
乌克兰语课程已完全融入 Vue 前端，作为 `features/courses/` 模块。课时数据以 JSON 格式存储在 `frontend/public/uk/*.json`，由 Vue 组件 `LessonRenderer` 渲染。
...
- 仅课时页有独立 URL：`https://mieltsm.top/uk/w3d1`
- 兼容性 redirect：访问 `/uk/` 时 vue-router 会写入对应 `activeTab` 后跳回 `/`（见 `router/index.ts`）
...
| `lesson.css` | `styles/course.css` + `styles/course-themes.css`（`data-course="ukrainian"` 切换主题色） |
...
课程由 Claude Code 在本地生成，输出 JSON 到 `frontend/public/uk/wXdY.json`，详见：
...
课程 JSON 文件随前端构建进入 `dist/uk/`，由标准 SPA `try_files` 伺服：
