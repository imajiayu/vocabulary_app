...
class BatchDefinitionService:
    ...
    def __init__(self, delay_between_requests=0.2, db_update_callback=None, num_workers=5):
        ...
        # 已更新的单词（用于轮询）
        self.updated_words: Dict[int, Any] = {}  # word_id -> definition
        self.updated_lock = threading.Lock()
    ...
    def get_updated_words(self) -> Dict[str, Any]:
        """
        获取已更新的单词并清空（用于轮询）

        Returns:
            dict: { "updates": [{ "id": int, "definition": dict }], "queue_size": int }
        """
        with self.updated_lock:
            updates = [{"id": word_id, "definition": definition}
                      for word_id, definition in self.updated_words.items()]
            ...
            while True:
                try:
                    ...
                    if definition:
                        ...
                        # 存储更新结果（用于轮询获取）
                        with self.updated_lock:
                            self.updated_words[word_id] = definition
