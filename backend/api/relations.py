...
def generate_relations():
    ...
    """
    try:
        from backend.services.relation_generation_manager import get_manager
        ...
        if success:
            return create_response(
                ...
                f"{relation_type} generation started. Use /generate/progress/{relation_type} to check progress."
            )
        else:
            return create_response(
                False,
...
@relations_bp.route("/generate/progress/<relation_type>", methods=["GET"])
def get_generation_progress(relation_type):
    """获取指定类型的生成任务进度（用于轮询）"""
    try:
        from backend.services.relation_generation_manager import get_progress
        ...
        progress = get_progress(relation_type)
        ...
        if progress:
            ...
            return create_response(
                True,
                {"relation_type": relation_type, "status": "not_started"},
                "No active task found"
            )

    except Exception as e:
        return (
            create_response(False, None, f"Failed to get progress: {str(e)}"),
            500,
        )


@relations_bp.route("/generate/progress", methods=["GET"])
def get_all_generation_progress():
    """获取所有生成任务的进度"""
    try:
        from backend.services.relation_generation_manager import get_all_progress
