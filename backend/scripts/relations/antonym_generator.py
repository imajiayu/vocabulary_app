...
class AntonymGenerator(BaseGenerator):
    ...
    def generate(
        ...
        progress.finish(f"找到 {total_found} 对反义词")
        ...
        print_stat("WordNet", stats_by_source['wordnet'])
        print_stat("手工定义", stats_by_source['manual'])
        print_stat("形态学", stats_by_source['morphological'])
