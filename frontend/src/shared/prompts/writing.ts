...
/** 逐段改进 — 输入第一版全文，返回每段对齐大纲的重写版 + 原文错误标注 + 是否修改 */
export const PARAGRAPH_IMPROVEMENT_PROMPT = `你是一位资深 IELTS 写作考官。请逐段分析以下 Task {taskType} 作文：为每段提供一个对齐大纲的重写版本，并在原文中标注出具体错误。

## 题目
{prompt}

## 学生大纲（作者的论点规划，重写时须沿用其立场与论点方向）
{outline}
...
[
  {
    ...
    "improved": "改进后的段落完整文本",
    "issues": [
      { "quote": "原文中有问题的片段（逐字复制原文，禁止改写）", "message": "一句话：错误类别 + 怎么改" }
    ],
    "changed": true
  }
]
...
2. improved: 完整的段落文本（不是差异描述），是一段对齐大纲的【重写版】。不必紧贴初稿原文——句式结构和用词都可以重写，目标是地道、流畅、贴近 IELTS 高分表达。只需做到：(a) 保留该段的核心大意（与原段大意相近，不跑题、不引入与原意无关的新内容）；(b) 与上方"学生大纲"的立场和论点方向保持一致。即使原段已经不错，也请给出一个更优的重写版本，不要原样复制原文。
3. issues: 数组，标注该段【原文】中存在的具体错误；若该段无错误，返回空数组 []。
   - quote: 必须是该段原文中【逐字出现】的连续子串（保持原始拼写、大小写、标点，禁止改写或标准化），尽量短、只覆盖出错的词或短语，便于在原文中精确定位。
   - message: 一句话中文说明，先点明错误类别（拼写 / 词汇 / 语法 / 语义），再说明问题与正确改法。例如「搭配错误：heightening 与 ability 不搭配，宜用 enhancing」。
   - 只标注真实错误（拼写、用词不当或搭配错误、语法/时态/单复数错误、语义不清或表意错误）；不要把"本身没错、只是可以更地道"的表达列为错误。
4. changed: 严格布尔值。若 improved 与原段落实质一致（忽略首尾空白、标点大小写差异），填 false；只要做了任何有意义的词汇/语法/结构调整，填 true。

改进与判断维度参考（用于发现问题、给出 improved 与 issues，但不要单独输出说明文字）：
...
其他要求：
- improved 须与学生大纲的立场和论点方向一致，并保留原段核心大意；句式与用词可自由重写，不必逐句对应原文
...
字段规则：
1. improved: 完整段落文本。可自由重写句式与用词，不必紧贴原文或当前改进版，只需保留段落核心大意并遵循用户指令
2. notes: 简要说明做了哪些改动
