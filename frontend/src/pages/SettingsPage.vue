...
const scrollToSection = (sectionId: string) => {
  ...
  if (element) {
    logger.log('[ScrollToSection] 开始滚动到:', sectionId)
    ...
    logger.log('[ScrollToSection] isScrollingProgrammatically 设置为 true')

    // 立即更新 activeSection
    activeSection.value = sectionId
    logger.log('[ScrollToSection] activeSection 已设置为:', sectionId)
    ...
    scrollTimeout = setTimeout(() => {
      ...
      logger.log('[ScrollToSection] 程序化滚动标记已清除')
      scrollTimeout = null
    }, 2000)
  }
}
...
const handleSaveSuccess = () => {
  logger.log('[SettingsPage] 设置保存成功')
}
